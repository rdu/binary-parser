...
Parser.prototype.arrayChoice = function(varName, options) {
    return this.setNextParser('arraychoice', varName, options);
};
...
Parser.prototype.generateArrayChoice = function(ctx)
{
    var name = ctx.generateVariable(this.varName);
    var array = ctx.generateOption(this.options.array);
    var value = ctx.generateTmpVariable();
    var item = ctx.generateTmpVariable();
    ctx.pushCode('{0} = [];', name);
    ctx.pushCode('{0} = {};', item);
    ctx.pushPath(this.varName);
    ctx.pushCode('for (var element in {0}) {', array);
    ctx.pushCode('{2} = {0}[element]["{1}"];', array, this.options.property, value);
    for (var choice in this.options.choices)
    {
        var type = this.options.choices[choice];
        ctx.pushCode('if ({1} == {0}) {', choice, value);
        ctx.pushScope(item);
        type.generate(ctx);
        ctx.popScope();
        ctx.pushCode('{0}.push({1});', name, item);
        ctx.pushCode('{0} = {};', item);
        ctx.pushCode("}");
    }
    ctx.popPath();
